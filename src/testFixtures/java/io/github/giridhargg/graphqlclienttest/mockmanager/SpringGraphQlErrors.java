 ...
 */
final class SpringGraphQlErrors {

    /** A {@code NOT_FOUND} error — the requested resource does not exist. */
    static GraphQLError notFound(String message, String... path) {
        return error(ErrorType.NOT_FOUND, message, path);
    }

    /** An {@code UNAUTHORIZED} error — the caller is not authenticated. */
    static GraphQLError unauthorized(String message, String... path) {
        return error(ErrorType.UNAUTHORIZED, message, path);
    }

    /** A {@code FORBIDDEN} error — the caller is authenticated but lacks permission. */
    static GraphQLError forbidden(String message, String... path) {
        return error(ErrorType.FORBIDDEN, message, path);
    }

    /** A {@code BAD_REQUEST} error — the request itself is invalid. */
    static GraphQLError badRequest(String message, String... path) {
        return error(ErrorType.BAD_REQUEST, message, path);
    }

    /** An {@code INTERNAL_ERROR} — an unclassified server-side failure. */
    static GraphQLError internal(String message, String... path) {
        return error(ErrorType.INTERNAL_ERROR, message, path);
    }
