 ...
 * tests, where a real Spring GraphQL server is started locally and the consumer's own {@code
 * WebClient}/{@code HttpGraphQlClient} (typically pointed at a network proxy such as WireMock,
 * which in turn forwards to the local server) is exercised unmodified.
 *
 * <p>Unlike {@link
 * io.github.giridhargg.graphqlclienttest.httpgraphqlclient.HttpGraphQlClientTest @HttpGraphQlClientTest}
 * — which replaces the {@code ClientHttpConnector} so requests never leave the JVM — this
 * annotation leaves the consumer's networking stack completely untouched. It instead wires a {@link
 * RuntimeWiringTestAutoConfiguration unified data fetcher} into Spring GraphQL's own {@code
 * RuntimeWiringConfigurer} machinery, so that whatever real HTTP request the consumer's application
 * makes, the server side of that request is served by the same {@link
 * io.github.giridhargg.graphqlclienttest.mockmanager.MockGraphQlServer} stubbing API used for
 * in-memory unit tests.
 *
 * <p>This is the right choice when consumers cannot substitute their production {@code
 * WebClient}/{@code RestClient} configuration in tests (e.g. it is wired deep inside business
 * logic) and instead need to intercept the request at the network boundary. - your {@code
 * application-integrationTest.yml} should have these minimal properties. Notice this file name,
 * that the spring profile must be '-integrationTest'
 *
 * <pre>{@code
 * spring:
 *   graphql:
 ...
 *
 * <pre>{@code @Bean
 * HttpGraphQlClient httpGraphQlClient( @Value("${my_graphql_server.host}") String host, @Value("${my_graphql_server.port}") String port, @Value("${my_graphql_server.path}") String path) {
 *   var baseUrl = String.format("%s:%s%s", host, port, path);
 *   var webClient = WebClient.create(baseUrl).mutate().build();
 ...
 *         graphQlServer.resolveFrom(GraphNode.of("bookById", GraphNode.of("id", "1")));
