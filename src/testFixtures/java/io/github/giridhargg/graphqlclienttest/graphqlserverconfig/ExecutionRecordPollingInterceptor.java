 ...
 * example, paginated fetches where the consumer's code under test makes several round-trips
 * within a single inbound request scope.
 *
 ...
 * Polling at that point would prematurely drop the queued expectation while nested fields are
 * still being resolved against it. Instead, {@link #tapSubscriptionCompletion} wraps the response's
 * event stream so the queue only advances once that stream completes or errors — i.e. once the
 * subscription itself is torn down, not once per emitted event.</p>
 *
 * <h2>Transport-level failures</h2>
 * <p>If {@code chain.next(request)} itself errors (a failure before any
 * {@code WebGraphQlResponse} is produced at all), the queue is still advanced via
 * {@code doOnError}, so a single broken request cannot stall a queue of otherwise-unrelated
 * expectations for the rest of the test.</p>
 */
class ExecutionRecordPollingInterceptor implements WebGraphQlInterceptor {
    ...
    ExecutionRecordPollingInterceptor(ObjectProvider<MockGraphQlServer> mockGraphQlServerObjectProvider) {
    ...
    /**
     * Wraps a subscription response's event {@link Publisher} so that
     * {@link MockGraphQlServer#pollExecutionRecord()} fires once the stream terminates
     * (completion or error), not when the stream is first established.
     */
    ...
    /**
     * Determines whether the selected operation in {@code request} is a {@code subscription},
     * by parsing the document and resolving the operation by name (falling back to the sole
     * operation when the document defines only one and no name was given, per the GraphQL spec).
     */
