...
/**
 * Enables in-memory GraphQL client testing for {@link org.springframework.graphql.client.HttpGraphQlClient}
 * consumers, without starting any local server or making any real network call.
 *
 * <p>Annotate a unit test class with this annotation to get:</p>
 * <ul>
 *   <li>An autowirable {@code HttpGraphQlClient} bean, backed entirely by an in-memory
 *       {@code graphql-java} engine</li>
 *   <li>An autowirable {@link io.github.giridhargg.graphqlclienttest.mockmanager.MockGraphQlServer}
 *       bean used to stub responses, errors, and subscription event streams per test</li>
 *   <li>Automatic per-test reset of all queued stubs (no expectation bleed between test methods,
 *       even without {@code @DirtiesContext})</li>
 * </ul>
 *
 * <pre>{@code
 ...
 *     @Test
 *     void fetchesBook() {
 *         graphQlServer.resolveFrom(QueryNode.of("bookById", QueryNode.of("id", "1", "name", "Dune")));
 ...
 * <p>By default, the GraphQL schema is auto-discovered from any {@code *.graphqls}/{@code *.graphql}
 * file on the test classpath. Override the search location via {@link #properties()} if your schema
 * lives somewhere non-standard, e.g.:</p>
 * <pre>{@code
 ...
 * convention and configuration properties.</p>
 *
...
public @interface HttpGraphQlClientTest {
    ...
    /**
     * Additional classes/configuration-classes to import alongside this annotation's own auto-configuration.
     */
     ...
     * "graphql.test.assets.schema-location=classpath:my-schema.graphqls")}</p>
     */
