...
import static io.github.giridhargg.graphqlclienttest.httpgraphqlclient.Constants.APQ_REGISTRY;
...
/**
 * Wires the in-memory {@link MockGraphQlServer} infrastructure for reactive, asynchronous {@link
 * HttpGraphQlClient} consumers.
 *
 * <p>This configuration is imported automatically by {@link
 * HttpGraphQlClientTest @HttpGraphQlClientTest} and is not normally referenced directly by
 * consumers. The resulting {@code HttpGraphQlClient} bean is wired against a {@link WebClient}
 * whose {@link org.springframework.http.client.reactive.ClientHttpConnector} executes every
 * outbound request entirely in-memory against an embedded {@code graphql-java} engine (see {@link
 * InMemoryMockGraphQlServer}) — no real network call is made and no local Spring GraphQL server is
 * started.
 *
 * <p>The bean wiring order is significant: {@code GraphQlTestExecutor} must compile the schema and
 * register the unified data fetcher before {@code MockGraphQlServer} attaches the in-memory {@code
 * ClientHttpConnector} to {@code WebClient.Builder}, since the connector captures the builder by
 * reference and mutates it in place. The {@code @DependsOn} chain below encodes that order.
 *
 * <p>Property resolution (schema location, persisted query locations) is owned entirely by {@link
 * HttpGraphQlClientTest @HttpGraphQlClientTest} via its {@code @TestPropertySource} composition;
...
class HttpGraphQlClientTestAutoConfiguration {
   ...
   * #mockGraphQlServiceServer} replaces the connector with an in-memory one before this builder is
   * used to construct the final {@link WebClient}.
   */
  @Primary
  ...
  /**
   * Creates the {@link MockGraphQlServer} and attaches its in-memory {@code ClientHttpConnector} to
   * {@link #webClientTestBuilder()}. This is the bean consumer tests inject to register request
   * expectations via {@code expect(...)} and resolvers via {@code resolveFrom(...)}.
   */
  @Primary
  ...
  MockGraphQlServer mockGraphQlServiceServer(
      @Qualifier(Constants.WEB_CLIENT_TEST_BUILDER) WebClient.Builder webClientBuilder,
      @Qualifier(Constants.GRAPHQL_TEST_EXECUTOR) GraphQlTestExecutor graphQlTestExecutor,
      @Qualifier(APQ_REGISTRY) ApqRegistry apqRegistry,
      ObjectMapper objectMapper) {
    return InMemoryMockGraphQlServer.createServer(
  ...
  /**
   * The {@link HttpGraphQlClient} consumer test code should autowire and exercise directly. Every
   * {@code document(...)} call this client makes is served entirely in-memory.
   */
  @Primary
  @Bean(Constants.HTTP_GRAPHQL_TEST_CLIENT)
  @DependsOn(Constants.WEB_TEST_CLIENT)
  HttpGraphQlClient httpGraphQlTestClient(
      @Qualifier(Constants.WEB_TEST_CLIENT) WebClient webClient) {
